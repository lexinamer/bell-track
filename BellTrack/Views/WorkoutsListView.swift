    ...
    var body: some View {
        List {
            ForEach(sortedDates, id: \.self) { date in
                Section {
                    // Date + optional note as the first row
                    DateHeader(
                        date: date,
                    ...
                    // Blocks with swipe actions
                    BlockRows(
                        blocks: groupedBlocks[date] ?? [],
                        onTapDay: { onEditDay(date) },
                        onDuplicateDay: { onDuplicateDay(date) },
                        onDeleteDay: { onDeleteDay(date) }
                    )
...
// Date + note "row" (tappable to edit the day)
...
#Preview {
    // Workout 1: Dec 31 with notes and 2 blocks
    let workout1Blocks = [
        WorkoutBlock(
            id: "1",
            userId: "preview",
    ...
    // Workout 2: Dec 30 with no notes and 1 block
    let workout2Block = [
        WorkoutBlock(
            id: "3",
            userId: "preview",
            date: Calendar.current.date(from: DateComponents(year: 2025, month: 12, day: 30))!,
            createdAt: Date(),
            name: "Plank",
            details: "x3 sets",
            isTracked: false,
    ...
    let allBlocks = workout1Blocks + workout2Block
    ...
    let grouped = Dictionary(grouping: allBlocks) {
